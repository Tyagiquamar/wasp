...
Wasp now serves a liveness check at `GET /up` in both development and production. It answers `200 OK` while the server is running.

Wasp used `/` for this before, and it behaved differently per environment:

- Development: `GET /` showed Wasp's wrong-port page.
- Production: `GET /` answered `200 OK`.

`/up` replaces both. The wrong-port page stays at `/` in development, registered after user `api`s, so an `api` at `/` wins over it. In production, `GET /` answers `404` unless you declare an `api` there.
